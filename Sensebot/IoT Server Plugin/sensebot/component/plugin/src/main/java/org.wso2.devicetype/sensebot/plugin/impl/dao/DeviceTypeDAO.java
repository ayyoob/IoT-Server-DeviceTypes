...
package org.wso2.devicetype.sensebot.plugin.impl.dao;
...
import org.wso2.devicetype.sensebot.plugin.impl.util.DeviceTypeUtils;
...
public class DeviceTypeDAO {
    ...
    public Device getDevice(String deviceId) throws DeviceMgtPluginException {
        ...
        try {
            conn = DeviceTypeDAOUtil.getConnection();
            ...
            if (resultSet.next()) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("sensebot device " + deviceId + " data has been fetched from " +
                            "sensebot database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while fetching sensebot device : '" + deviceId + "'";
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
            DeviceTypeUtils.cleanupResources(stmt, resultSet);
            DeviceTypeDAOUtil.closeConnection();
        }
        return iotDevice;
    }
    ...
    public boolean addDevice(Device device) throws DeviceMgtPluginException {
        ...
        try {
            ...
            if (rows > 0) {
                status = true;
                if (log.isDebugEnabled()) {
                    log.debug("sensebot device " + device.getDeviceIdentifier() + " data has been" +
                            " added to the sensebot database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while adding the sensebot device '" +
                    device.getDeviceIdentifier() + "' to the sensebot db.";
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
    ...
    public boolean deleteDevice(String deviceId) throws DeviceMgtPluginException {
        ...
        try {
            ...
            if (rows > 0) {
                status = true;
                if (log.isDebugEnabled()) {
                    log.debug("sensebot device " + deviceId + " data has deleted" +
                            " from the sensebot database.");
                }
            }
        } catch (SQLException e) {
            String msg = "Error occurred while deleting sensebot device " + deviceId;
            log.error(msg, e);
            throw new DeviceMgtPluginException(msg, e);
        } finally {
            DeviceTypeUtils.cleanupResources(stmt, null);
